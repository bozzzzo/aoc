...
def gen_rot(b):
    ...
    return scanner_id, list(dict(sorted([(tuple(rot90(rot90(rot90(beacon, i, 2), j, 1), k, 0) for beacon in beacons), (i, j, k)) for i in range(4) for j in range(4) for k in range(4)], key=lambda x:sum(x[1]))).values())
